...
import { login, logout, oidcAuthorize, oidcResponse, oauth2Authorize, oauth2Response, signup } from '../middleware'
...
// logout
router.get('/logout', logout({ returnTo: config.siteUrl }))
...
router.get('/google/authorize', oidcAuthorize({
	...
	discoveryUrl: config.google.discoveryUrl,
	returnTo: config.siteUrl
}))
...
router.get('/discord/authorize', oauth2Authorize({
	...
	authorizeUrl: config.discord.authorizeUrl,
	tokenUrl: config.discord.tokenUrl,
	returnTo: config.siteUrl
}))
router.get('/discord/response', [
	oauth2Response({
		clientId: config.discord.clientId,
		clientSecret: config.discord.clientSecret,
		redirectUrl: config.discord.redirectUrl,
		...
		authorizeUrl: config.discord.authorizeUrl,
		tokenUrl: config.discord.tokenUrl,
		userinfoUrl: config.discord.userinfoUrl,
		guildsUrl: config.discord.guildsUrl,
		returnTo: config.siteUrl
	})
])
